<script>
  ...
  function exportPng() {
    ...
    for (const layer of layers) {
      ...
      layerCtx.drawImage(maskSource, 0, 0, width, height);
  ...
  function contoursToSvgPathMarkup(
    paths,
    _sourceWidth,
    sourceHeight,
    scaleX,
    ...
    return mappedPaths.map(
      (pathData) => `  <path d="${pathData}" fill="${color}" stroke="none"/>`,
    );
  }
  ...
  function createRandomCircleMap(sourceGridX = gridX, sourceGridY = gridY) {
    ...
    for (const diameter of diameterList) {
      ...
      for (let attempt = 0; attempt < 200 && !placed; attempt += 1) {
        ...
        placed = hasMargin && addCircleAt(xHalf, yHalf, diameter, nextCircles, sourceGridX, sourceGridY);
      }
    }
  ...
  function updateMaskCanvas(sourceCircles = circles, sourceGridX = gridX, sourceGridY = gridY) {
    ...
    const sourceWidthPx = sourceGridX * cellSize;
    const sourceHeightPx = sourceGridY * cellSize;
    const originX = (canvasWidthPx - sourceWidthPx) / 2;
    const originY = (canvasHeightPx - sourceHeightPx) / 2;
  ...
  function resizeRenderer(width, height) {
    ...
    const scale = getEffectiveRenderScale(safeWidth, safeHeight, getRenderScale());
  ...
  <section class="panel">
    ...
    <div class="controls">
      <label>
        ...
        <div class="input-row">
          <input type="range" min="20" max="200" step="10" bind:value={cellSize} />
          <input class="number" type="number" min="20" max="200" step="10" bind:value={cellSize} />
        </div>
